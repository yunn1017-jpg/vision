...
1_드래그 시각화: 마우스 이동에 따라 실시간으로 초록색 가이드 사각형 표시
    ...
    if event == cv.EVENT_LBUTTONDOWN: # 마우스 왼쪽 버튼을 눌렀을 때
        ...
        ix, iy = x, y # 처음 마우스를 클릭한 시점의 좌표를 ix, iy에 저장함
        
    # 3. 드래그하며 사각형 시각화 (cv.rectangle)
    elif event == cv.EVENT_MOUSEMOVE: # 마우스가 움직일 때
        if drawing == True: # 마우스가 눌린 상태(드래그 중)라면
            original_img = clone.copy() # 잔상이 남지 않도록 깨끗한 원본 복사본을 덮어씌움
            # 시작점(ix,iy)부터 현재 마우스 좌표(x,y)까지 초록색(0,255,0) 선 두께 2의 사각형을 그림
            cv.rectangle(original_img, (ix, iy), (x, y), (0, 255, 0), 2) 

2_영역 추출: 마우스를 떼는 순간 해당 영역을 슬라이싱하여 새 창에 표시

    # 4. 마우스를 놓았을 때 ROI 추출 (Numpy 슬라이싱)
    elif event == cv.EVENT_LBUTTONUP: # 마우스 왼쪽 버튼을 놓았을 때
        drawing = False # 드래그 상태 종료
        cv.rectangle(original_img, (ix, iy), (x, y), (0, 255, 0), 2) # 최종 선택된 위치에 사각형을 그림
        
        # 드래그 방향(우하향, 좌상향 등)에 상관없이 좌표가 제대로 잡히도록 min/max로 X, Y 시작과 끝 좌표 정렬
        start_x, end_x = min(ix, x), max(ix, x)
        start_y, end_y = min(iy, y), max(iy, y)
        
        # 시작점과 끝점이 다른 경우에만 (점 클릭이 아니라 실제로 드래그를 한 경우)
        if start_x != end_x and start_y != end_y:
            # 원본 이미지(clone)에서 해당 좌표만큼 numpy 슬라이싱으로 잘라내어 roi_selected 변수에 저장 [y범위, x범위]
            roi_selected = clone[start_y:end_y, start_x:end_x]
            # 잘라낸 ROI 이미지를 'Extracted ROI'라는 별도의 새 창에 띄워 출력함
            cv.imshow('Extracted ROI', roi_selected) 
...
3_저장 및 리셋: s 키로 추출된 영역 저장, r 키로 초기화
    ...
    # 5. 'r' 키를 누르면 영역 리셋
        ...
        original_img = clone.copy() # 메인 이미지를 초록색 사각형이 없는 깨끗한 원본 복사본으로 되돌림
        try:
            cv.destroyWindow('Extracted ROI') # 혹시 띄워져 있을지 모르는 'Extracted ROI' 창을 닫음
        except:
            pass # 창이 없어서 에러가 발생하면 무시하고 넘어감 (pass)
            
    # 6. 's' 키를 누르면 이미지 파일로 저장 (cv.imwrite)
    elif key == ord('s'): # 's' 키를 누르면
        if roi_selected is not None: # 선택된 ROI(잘라낸 이미지)가 존재한다면
