...
✨ 핵심 코드
이미지 색상 공간 변환 (BGR → Gray)

Python
gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)# 그레이스케일 변환
두 이미지 배열을 가로로 결합 (Stacking)

Python
combined = np.hstack((img, gray_bgr))# 이미지 가로 연결
2️⃣ 2.py: 마우스 드로잉 그림판 (Paint Mode)
...
Python
    ...
    color = (0, 0, 255) # 빨간색
    ix, iy = x, y # 클릭 시작 지점 저장
키보드 조작: +/= 키로 붓 크기 증가, - 키로 감소 (최소 1 ~ 최대 15)

Python
elif key == ord('+') or key == ord('='): # 입력된 키가 '+' 일 경우
    brush_size += 1 # 붓 크기를 1 증가시킴
    if brush_size > 15: brush_size = 15 # 최대 제한
elif key == ord('-'): # 입력된 키가 '-' 일 경우
    brush_size -= 1 # 붓 크기를 1 감소시킴
    if brush_size < 1: brush_size = 1 # 최소 제한
3️⃣ 3.py: 마우스 드래그를 이용한 ROI 추출
...
✨ 핵심 코드
드래그 시각화: 마우스 이동에 따라 실시간으로 초록색 가이드 사각형 표시
...
if event == cv.EVENT_LBUTTONDOWN:
    ...
    if drawing == True:
        original_img = clone.copy()
        cv.rectangle(original_img, (ix, iy), (x, y), (0, 255, 0), 2) 
영역 추출: 마우스를 떼는 순간 해당 영역을 슬라이싱하여 새 창에 표시
...
저장 및 리셋: s 키로 추출된 영역 저장, r 키로 초기화

Python
elif key == ord('r'): # 리셋
    original_img = clone.copy()
elif key == ord('s'): # 저장
    cv.imwrite('saved_roi.jpg', roi_selected)